...
- Execute `storage-002b` backup/import/overwrite and encryption workflows, then complete the `storage-002` integration gate before connecting persistence and chat streaming through real storage implementations.
- `storage-002`: the catalog, repair, dynamic FTS, and projection slices (`storage-002a-1` through `storage-002a-3`) are done; `storage-002b` and the final integration gate remain.
